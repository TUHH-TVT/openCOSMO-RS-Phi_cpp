...
molecule getMoleculeFromJSONfile(std::string& path) {

    std::ifstream jsonFile(path);
    if (jsonFile.fail())
        throw std::runtime_error("The required input json file path could not be opened. Does it exists? Is the path correct? Is it a valid json file?");
    json inputFileData = json::parse(jsonFile);

    molecule newMolecule;

    newMolecule.qmMethod = inputFileData["qm_method"];
    newMolecule.name = path;
    newMolecule.epsilonInfinityTotalEnergy = 0;
    newMolecule.atomAtomicNumbers = Eigen::VectorXi();

    newMolecule.Volume = double(inputFileData["volume"]);

    newMolecule.atomPositions = Eigen::MatrixXd::Zero(0, 3);
    newMolecule.atomAtomicNumbers = Eigen::VectorXi();
    newMolecule.atomRadii = Eigen::VectorXd();

    param.sw_atomicNumber = 0;
    param.sw_differentiateHydrogens = 0;
    param.sw_differentiateMoleculeGroups = 0;

    double moleculeArea = 0;
    double totalScreeningCharge = 0;
    unsigned short moleculeGroup = 0;

    auto segmentTypesInfo = inputFileData["segment_types"].template get<std::vector<std::vector<double>>>();
    for (int j = 0; j < segmentTypesInfo.size(); j++) {
        double sigma = segmentTypesInfo[j][0];
        double sigmaCorr = segmentTypesInfo[j][1];
        double area = segmentTypesInfo[j][2];

        moleculeArea += area;
        totalScreeningCharge += area * sigma;
        unsigned short segmentTypeHydrogenBondingType = sigma < 0 ? 1 : 2;
        unsigned short atomicNumber = 0;
        newMolecule.segments.add(0, moleculeGroup, sigma, sigmaCorr, segmentTypeHydrogenBondingType, atomicNumber, area, newMolecule.index);
    }

    newMolecule.Area = moleculeArea;
    newMolecule.moleculeCharge = (signed char)(std::round(-1.0f * totalScreeningCharge)); ;
    return newMolecule;
}
